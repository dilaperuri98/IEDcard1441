...
const players = [{name:"Player1",position: 0,color: "grey"},{name:"Player2",position: 0,color: "white"}];
...
for (var y = height; y >= 0; y--) {
  ...
  for (var x = 0; x < width; x++) {
    ...
    row.push({x,y,occupied:null,position,color: blackSquare ? "yellow" : "purple"});
...
const renderBoard = ()=>{
  ...
  ladders.forEach(ladder=>{
    ...
    drawLine({color : isLadder ? "blue" : "red",startPos,endPos});
  });
